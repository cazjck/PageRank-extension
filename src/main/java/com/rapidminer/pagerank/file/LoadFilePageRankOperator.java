...
*/
package com.rapidminer.pagerank.file;
...
public class LoadFilePageRankOperator extends Operator {
	...
	public LoadFilePageRankOperator(OperatorDescription description) {
		super(description);
		...
		getTransformer().addRule(new MDTransformationRule() {
			...
			public void transformMD() {
				try {
					...
					addError(new SimpleProcessSetupError(Severity.WARNING, getPortOwner(), "passthrough",
							e.getMessage()));
				}
			}
		});
	...
	protected void checkMetaData() throws UserError {
		...
		try {
			...
			case SOURCE_TYPE_URL:
				// check only if url is valid, not if it's accessible for
				// performance reasons
				try {
					// ignore this warning - only create URL to check if the
					// parameter string
	...
	public void doWork() throws OperatorException {
		...
		case SOURCE_TYPE_FILE:
			...
			/*
			 * ArrayList<ArrayList<String>> arrayList; String line;
			 * 
			 * FileInputStream fis = (FileInputStream) result.openStream();
			 * InputStreamReader is = new InputStreamReader(fis); BufferedReader
			 * br = new BufferedReader(is); arrayList = new ArrayList<>();
			 * 
			 * try { while ((line = br.readLine()) != null) { String[] s =
			 * line.split("\t"); ArrayList<String> arr = new ArrayList<>(); for
			 * (int i = 0; i < s.length; i++) { arr.add(s[i]); }
			 * arrayList.add(arr); } } catch (IOException e1) { // TODO
			 * Auto-generated catch block e1.printStackTrace(); }
			 */
			try {
				ExampleSet exampleSet = HadoopCluster.getDataHadoopLocal(file.getPath());
				result.getAnnotations().setAnnotation(Annotations.KEY_SOURCE, source);
				fileOutputPort.deliver(exampleSet);
				break;
			} catch (Exception e1) {
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterTypeFile parameterTypeFile = new ParameterTypeFile(PARAMETER_FILENAME, "File to open", null, true,
				false);
		parameterTypeFile.registerDependencyCondition(
				new EqualTypeCondition(this, PARAMETER_SOURCE_TYPE, SOURCE_TYPES, true, SOURCE_TYPE_FILE));
		parameterTypes.add(parameterTypeFile);

		ParameterTypeString parameterTypeUrl = new ParameterTypeString(PARAMETER_URL, "URL to open", true, false);
		parameterTypeUrl.registerDependencyCondition(
				new EqualTypeCondition(this, PARAMETER_SOURCE_TYPE, SOURCE_TYPES, true, SOURCE_TYPE_URL));
		...
		parameterTypeRepositoryLocation.registerDependencyCondition(
				new EqualTypeCondition(this, PARAMETER_SOURCE_TYPE, SOURCE_TYPES, true, SOURCE_TYPE_REPOSITORY));
